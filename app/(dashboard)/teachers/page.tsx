...
export default function TeachersPage() {
  ...
  const [photoTeacher, setPhotoTeacher] = useState<Teacher | null>(null);
  ...
  const handleDelete = async () => {
    ...
    if (!confirm(`Delete ${selected.size} teacher(s)? This cannot be undone.`)) return;
  ...
  const toggleAll = () => {
    if (selected.size === teachers.length) {
      setSelected(new Set());
    } else {
      setSelected(new Set(teachers.map((tc) => tc._id)));
    }
  };

  const closeModal = () => { setModal("none"); setPhotoTeacher(null); setProfileTeacher(null); };
  ...
  return (
    <div className="animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800">{t("teachers.title")}</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {pagination.total} {t("teachers.totalTeachers")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="py-2 px-4 rounded-xl border border-indigo-300/50 text-indigo-600 text-sm font-semibold hover:bg-indigo-50 transition flex items-center gap-2 cursor-pointer"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="py-2 px-4 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 text-white text-sm font-semibold shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:brightness-110 transition-all flex items-center gap-2 cursor-pointer"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 mb-4">
        <div className="flex flex-wrap gap-3">
          <input
            ...
            className="flex-1 min-w-[200px] px-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300/50 focus:border-indigo-400"
          />
          <select value={deptFilter} onChange={(e) => setDeptFilter(e.target.value)} className="px-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300/50 cursor-pointer">
            <option value="">{t("common.all")} {t("teachers.department")}</option>
            {DEPARTMENTS.map((d) => <option key={d} value={d}>{d}</option>)}
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300/50 cursor-pointer">
            <option value="">{t("common.all")} {t("common.status")}</option>
            {["Active", "Inactive", "On Leave", "Resigned"].map((s) => <option key={s} value={s}>{tStatus("teacherStatus", s)}</option>)}
          </select>
          {selected.size > 0 && (
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="py-3 px-4 text-left">
                  <input type="checkbox" checked={teachers.length > 0 && selected.size === teachers.length} onChange={toggleAll} className="rounded border-slate-300 cursor-pointer" />
                </th>
                {[t("teachers.teacherId"), t("common.name"), t("teachers.subject"), t("teachers.department"), t("teachers.contact"), t("teachers.qualification"), t("common.status"), ""].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="py-16 text-center">
                  ...
                  <p className="text-slate-500 font-medium">{t("teachers.noTeachers")}</p>
                  <p className="text-slate-400 text-xs mt-1">{t("teachers.noTeachersHint")}</p>
                </td></tr>
              ) : teachers.map((tchr) => (
                <tr key={tchr._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition">
                  <td className="py-3 px-4">
                    <input type="checkbox" checked={selected.has(tchr._id)} onChange={() => toggleSelect(tchr._id)} className="rounded border-slate-300 cursor-pointer" />
                  </td>
                  <td className="py-3 px-3 text-slate-600 font-mono text-xs whitespace-nowrap">{tchr.teacherId}</td>
                  <td className="py-3 px-3 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <button
                        onClick={() => { setPhotoTeacher(tchr); setModal("photo"); }}
                      ...
                      >
                        {tchr.profilePicture ? (
                          ...
                          <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${getGradient(tchr.firstName + tchr.lastName)} flex items-center justify-center text-white text-xs font-bold`}>
                            {getInitials(tchr.firstName, tchr.lastName)}
                          </div>
                        )}
                      ...
                      <div>
                        <p className="text-slate-800 font-medium text-sm">{tchr.firstName} {tchr.lastName}</p>
                        {tchr.email && <p className="text-slate-400 text-xs">{tchr.email}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-3 text-slate-600">{tSubject(tchr.subject)}</td>
                  <td className="py-3 px-3 text-slate-600">{tchr.department}</td>
                  <td className="py-3 px-3 text-slate-600 whitespace-nowrap">{tchr.phone || "—"}</td>
                  <td className="py-3 px-3 text-slate-600 whitespace-nowrap text-xs">{tchr.qualification || "—"}</td>
                  <td className="py-3 px-3">
                    <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${statusColors[tchr.status] || "bg-slate-100 text-slate-500"}`}>
                      {tStatus("teacherStatus", tchr.status)}
                    </span>
                  </td>
                  <td className="py-3 px-3">
                    <button
                      onClick={() => { setProfileTeacher(tchr); setModal("profile"); }}
                      className="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-semibold text-indigo-600 hover:bg-indigo-50 transition cursor-pointer"
                    >
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(pagination.totalPages, 5) }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-bold flex items-center justify-center cursor-pointer ${p === pagination.page ? "bg-indigo-500 text-white" : "text-slate-500 hover:bg-slate-100"}`}
                >{p}</button>
              ))}
              <button
                onClick={() => fetchTeachers(pagination.page + 1)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={closeModal}>
          <div className={`bg-white rounded-2xl shadow-2xl w-full ${modal === "profile" ? "max-w-xl" : "max-w-2xl"} max-h-[90vh] overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
            {modal === "add" && (
              <AddTeacherForm
                onClose={closeModal}
                onSuccess={(msg) => { closeModal(); setMessage({ type: "success", text: msg }); fetchTeachers(); }}
              />
            )}
            {modal === "upload" && (
              <UploadExcel
            ...
            {modal === "photo" && photoTeacher && (
              <StudentPhotoCapture
                currentPhoto={photoTeacher.profilePicture || ""}
                studentName={`${photoTeacher.firstName} ${photoTeacher.lastName}`}
                onSave={async (base64) => {
                  const res = await fetch(`/api/teachers/${photoTeacher._id}/photo`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                  ...
                  closeModal();
                  setMessage({ type: "success", text: "Teacher photo updated!" });
                  fetchTeachers(pagination.page);
                }}
                onClose={closeModal}
              />
            )}
            {modal === "profile" && profileTeacher && (
              <TeacherProfile
                teacher={profileTeacher}
                onClose={closeModal}
                onUpdate={(updated) => {
                  setProfileTeacher(updated);
                  setTeachers((prev) => prev.map((tc) => tc._id === updated._id ? updated : tc));
                }}
                onPhotoClick={() => {
                  setPhotoTeacher(profileTeacher);
                  setModal("photo");
                }}
              />
            )}
...
/* ─── Add Teacher Form ─── */
function AddTeacherForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: (msg: string) => void }) {
  const [form, setForm] = useState({
    teacherId: "", firstName: "", lastName: "", email: "", phone: "",
    dateOfBirth: "", gender: "Male", subject: "", department: "",
    qualification: "", experience: "", address: "", salary: "", status: "Active",
  });
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          { label: "Teacher ID*", field: "teacherId", placeholder: "T100001", required: true },
          { label: "First Name*", field: "firstName", placeholder: "Robert", required: true },
          { label: "Last Name*", field: "lastName", placeholder: "Williams", required: true },
          { label: "Email", field: "email", placeholder: "robert@school.com", type: "email" },
          { label: "Phone", field: "phone", placeholder: "555-0201" },
          { label: "Date of Birth", field: "dateOfBirth", type: "date" },
          { label: "Subject*", field: "subject", placeholder: "Mathematics", required: true },
          { label: "Department*", field: "department", placeholder: "Science", required: true },
          { label: "Qualification", field: "qualification", placeholder: "M.Sc Mathematics" },
          { label: "Experience", field: "experience", placeholder: "10 years" },
          { label: "Salary", field: "salary", placeholder: "55000" },
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.summary.inserted > 0) {
        setTimeout(() => onSuccess(`${data.summary.inserted} teacher(s) imported successfully!`), 1500);
      }
    } catch {
    ...
    <div>
      <div className="flex items-center justify-between p-6 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800">Upload Teacher Data</h3>
            <p className="text-xs text-slate-500">Import teachers from an Excel or CSV file</p>
          </div>
        </div>
      ...
      <div className="p-6">
        <div
          className={`border-2 border-dashed rounded-2xl p-8 text-center transition-colors cursor-pointer ${dragOver ? "border-indigo-400 bg-indigo-50" : "border-slate-200 hover:border-slate-300"}`}
        ...
        >
          ...
          {file ? (
            ...
            <>
              ...
              <p className="text-sm text-slate-600 font-medium">Drag & drop your file here, or <span className="text-indigo-600 font-semibold cursor-pointer">browse</span></p>
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-slate-500">
          ...
          Need help formatting? <a href="/api/teachers/template" download className="text-indigo-600 font-semibold hover:underline">Download the template</a>
        </div>

        {error && <div className="mt-4 p-3 rounded-xl bg-rose-50 border border-rose-200 text-sm text-rose-600 font-medium">{error}</div>}
          ...
          <div className="mt-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="text-center p-3 rounded-lg bg-rose-50">
                <div className="text-xl font-bold text-rose-600">{result.errored}</div>
        ...
        {validationErrors.length > 0 && (
          <div className="mt-4 p-4 rounded-xl bg-rose-50/50 border border-rose-100 max-h-40 overflow-y-auto">
            <h4 className="text-xs font-bold text-rose-600 mb-2">Validation Errors</h4>
            {validationErrors.map((err, i) => (
              <p key={i} className="text-xs text-slate-600 py-0.5">
                <span className="font-semibold text-rose-600">Row {err.row}:</span> {err.message}
              </p>
            ))}
          </div>
        )}
      ...
      <div className="flex justify-end gap-3 p-6 border-t border-slate-100">
        ...
        <button
          ...
          className="py-2.5 px-6 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 text-white text-sm font-semibold shadow-lg shadow-indigo-500/25 hover:brightness-110 transition-all disabled:opacity-60 cursor-pointer"
        >
          {uploading ? (
