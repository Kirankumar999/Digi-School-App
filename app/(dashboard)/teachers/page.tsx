...
export default function TeachersPage() {
  ...
  const toggleAll = () => {
    if (selected.size === teachers.length) {
      ...
      setSelected(new Set(teachers.map((t) => t._id)));
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {pagination.total} total teacher{pagination.total !== 1 ? "s" : ""}
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 mb-4">
        <div className="flex flex-wrap gap-3">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300/50 cursor-pointer">
            ...
            {["Active", "Inactive", "On Leave", "Resigned"].map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          {selected.size > 0 && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                {["Teacher ID", "Name", "Subject", "Department", "Contact", "Qualification", "Status", ""].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="py-16 text-center">
                  ...
                  <p className="text-slate-400 text-xs mt-1">Add teachers manually or upload an Excel file</p>
                </td></tr>
              ) : teachers.map((t) => (
                <tr key={t._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition">
                  <td className="py-3 px-4">
                    <input type="checkbox" checked={selected.has(t._id)} onChange={() => toggleSelect(t._id)} className="rounded border-slate-300 cursor-pointer" />
                  </td>
                  <td className="py-3 px-3 text-slate-600 font-mono text-xs whitespace-nowrap">{t.teacherId}</td>
                  <td className="py-3 px-3 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <button
                        onClick={() => { setPhotoTeacher(t); setModal("photo"); }}
                        className="relative group cursor-pointer shrink-0"
                        title="Click to update photo"
                      >
                        {t.profilePicture ? (
                          <img src={t.profilePicture} alt={t.firstName} className="w-8 h-8 rounded-full object-cover ring-2 ring-slate-100" />
                        ) : (
                          <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${getGradient(t.firstName + t.lastName)} flex items-center justify-center text-white text-xs font-bold`}>
                      ...
                      <div>
                        <p className="text-slate-800 font-medium text-sm">{t.firstName} {t.lastName}</p>
                        {t.email && <p className="text-slate-400 text-xs">{t.email}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-3 text-slate-600">{t.subject}</td>
                  <td className="py-3 px-3 text-slate-600">{t.department}</td>
                  <td className="py-3 px-3 text-slate-600 whitespace-nowrap">{t.phone || "—"}</td>
                  <td className="py-3 px-3 text-slate-600 whitespace-nowrap text-xs">{t.qualification || "—"}</td>
                  <td className="py-3 px-3">
                    <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${statusColors[t.status] || "bg-slate-100 text-slate-500"}`}>
                      {t.status}
                    </span>
                  </td>
                  <td className="py-3 px-3">
                    <button
                      onClick={() => { setProfileTeacher(t); setModal("profile"); }}
